import tmdbv3api
import requests
from DBAuxiliaryFuncs import DBAuxiliaryFuncs
import mysql.connector
from datetime import datetime as dt

movie_titles = set()
db_aux = DBAuxiliaryFuncs()
...
class FillingTables:
    ...
    def fill_genres_table(self):
        ...
        for genre in genres.movie_list():
            db_aux.insert_genre(genre["name"])

    def fill_rated_table(self):
        for rating in content_rating:
            db_aux.insert_rating(rating)
    ...
    def __fill_movie_related_table(self, tmdb_movie):
        ...
        # fill the tables
        db_aux.insert_movie_table(tmdb_movie, omdb_movie)
    ...
    def __insert_movies_genres_table(self, tmdb_movie):  # len(tmdb_movie.genres) > 0
        for g in tmdb_movie.genres:
            name = g["name"]
            db_aux.insert_movies_genres(tmdb_movie.id, self._genres[name])

    def __insert_movies_countries_table(self, tmdb_movie):
        for country in tmdb_movie.production_countries:
            name = country["name"]
            db_aux.insert_movies_countries(tmdb_movie.id, self._countries[name])
    ...
    def __insert_companies_tables(self, tmdb_movie):
        for comp in tmdb_movie.production_companies:
            ...
            if name.lower() not in self._companies:
                db_aux.insert_company(name)
                self._companies[name.lower()] = db_aux.get_company_id(name)[0][0]
            db_aux.insert_movies_companies(tmdb_movie.id, self._companies[name.lower()])
    ...
    def __insert_movie_score_table(self, omdb_movie, tmdb_movie):
        ...
        db_aux.insert_score(tmdb_movie.id, imdb_rating, tomatoes_rating, metacritic_rating,
    ...
    def __fill_actors_and_person_tables(self, cast, movie_id):
        ...
        for ac in cast:
            ...
            db_aux.insert_movies_actors(movie_id, self._persons[name], figure)
            counter += 1
    ...
    def __fill_crew_and_persons_table(self, crew, movie_id):
        ...
        for cr in crew:
            ...
            if job is not None and ((not director_flag and job == "Director") or (not writer_flag and job == "Writer")):
                ...
                db_aux.insert_movies_crew(movie_id, self._persons[name], job)
                if job == "Director":
                    director_flag = True
                else:
                    writer_flag = True
    ...
    def __fill_person_table_and_dict(self, per):
        ...
        if poster is None or len(poster) < 3:
            poster = None
        db_aux.insert_person(per.name.split()[0], per.name.split()[1], gender, poster)
        self._persons[per.name] = db_aux.get_person_id(per.name.split()[0], per.name.split()[1])[0][0]


fill_it = FillingTables()

# NOTICE - those 3 calls should be executed before calling fill_all_the_tables
# fill_it.fill_genres_table()
# fill_it.fill_rated_table()
# fill_it.fill_production_countries_table()

fill_it.fill_all_the_tables(0, 0)
